...
func main() {
	...
	log.Println("Generating random account...")
	pvtKey, randomAccount := goethereumhelper.NewAccount()

	log.Println("Generating BJJ wallet for the address: ", randomAccount.Hex())
	bjjWallet, _, err := account.CreateBjjWalletWithAccCreationSignatureFromPvtKey(
		pvtKey, networkDefinition.ChainID, networkDefinition.RollupContractAddress.Hex())
	if err != nil {
